...
NMAX = 9
case = '2par_one_nonlinear_constraint'
c = -1.5
...
def plot_rosenbrock(iter=1, ax=None):
    ...
    constraint_y = 2.25 * constraint_x**2 + c
    ...
    ax.scatter(current_point[0], current_point[1], c='b', marker='o', s=40, zorder=20, label=f'base: ({par1:.4f}, {par2:.4f})')
    ...
    # Ensemble members
    pars = pars.values
    ax.scatter(pars[:, 0], pars[:, 1], c='w', marker='o', s=25, zorder=10, label='')
    # Candidate points
    if (iter < NMAX):
    ...
    ax.set_title('Constrained Rosenbrock Function (-2.25*x1^2 + x2 <= ' + str(c) + ')\n Ensemble PESTPP-SQP - Iteration = ' + str(iter))
