...
NMAX = 8
case = '2par_three_linear_constraint_infeas'
...
def plot_rosenbrock(iter=1, ax=None):
    ...
    C3 = np.zeros_like(X)  # Array to store third constraint values
    for i in range(len(x)):
        for j in range(len(y)):
            ...
            C3[j, i] = Y[j, i] - 0.5*X[j, i]  # Third constraint values
    ...
    # Plot contours of the third constraint function
    constraint_contour3 = ax.contour(X, Y, C3, levels=constraint_levels, colors='orange', linestyles='dashed', linewidths=1)
    ax.clabel(constraint_contour3, inline=True, fontsize=8, fmt='%1.1f')
    ...
    # Plot the third constraint line: x[1] + 0.5*x[1] = c3
    constraint3_y = 0.5 * constraint_x + c3
    ax.plot(constraint_x, constraint3_y, ls='-', color='orange', linewidth=2, label='Constraint 3: x2 + 0.5*x1 = ' + str(c3))
    ...
    ax.fill_between(constraint_x, constraint3_y, -5, alpha=0.2, color='orange', label='Infeasible region 3')
